...
class DataVisualizationAgent(BaseAgent):
    ...
    from data_science_tools_agents.agents import DataVisualizationAgent

    llm = ChatOpenAI(model="gpt-4o-mini")

    data_visualization_agent = DataVisualizationAgent(
        model=llm, n_samples=30, log=True, log_path="logs", human_in_the_loop=True
    )
...
def make_data_visualization_agent(
    ...
    import pandas as pd
    from langchain_openai import ChatOpenAI
    from data_science_tools_agents.agents import data_visualization_agent
    ...
    prompt_text_human_review = (
        "Are the following data visualization instructions correct? (Answer 'yes' or provide modifications)\n{steps}"
    )
