...
def mlflow_search_experiments(
    filter_string: Optional[str] = None, tracking_uri: str | None = None, registry_uri: str | None = None
) -> str:
...
def mlflow_predict_from_run_id(
    run_id: str, data_raw: Annotated[dict, InjectedState("data_raw")], tracking_uri: Optional[str] = None
) -> tuple:
...
@tool(response_format="content")
def mlflow_launch_ui(port: int = 5000, host: str = "localhost", tracking_uri: Optional[str] = None) -> str:
...
@tool(response_format="content_and_artifact")
def mlflow_list_artifacts(run_id: str, path: Optional[str] = None, tracking_uri: Optional[str] = None) -> tuple:
    ...
    for artifact in artifact_list:
        artifacts_data.append({"path": artifact.path, "is_dir": artifact.is_dir, "file_size": artifact.file_size})
...
def mlflow_download_artifacts(
    ...
    message = (
        f"Artifacts for run_id='{run_id}' have been downloaded to: {local_path}. Total files: {len(downloaded_files)}."
    )
...
def mlflow_search_registered_models(
    ...
    client = MlflowClient(tracking_uri=tracking_uri, registry_uri=registry_uri)
    models = client.search_registered_models(filter_string=filter_string, order_by=order_by, max_results=max_results)
