...
class BaseAgent(CompiledStateGraph):
    ...
    async def ainvoke(self, input: Union[dict[str, Any], Any], config: Optional[RunnableConfig] = None, **kwargs):
...
def node_func_human_review(
    ...
    user_input = interrupt(
        value=prompt_text.format(steps=state.get(recommended_steps_key, "") + "\n\n" + code_markdown)
    )
...
def node_func_fix_agent_code(
    ...
    return {code_snippet_key: response, error_key: None, retry_count_key: state.get(retry_count_key) + 1}
